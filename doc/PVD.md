...
| **Self-quality** | `elc` run against its own source reports no function exceeding a cyclomatic complexity of 15, and no critical finding from a band other than recursion, fan-out and maintainability. Two debts are recorded rather than gated: some sixty functions sit in §A.3's warning band between 11 and 15, and sixteen fall below §A.6's critical maintainability score — four of those being the artefact §A.6 describes, three from routing every diagnostic through one function and one from routing every name comparison through one reduction, which HLR-200 requires. The second is ratcheted — it may fall but not rise. Both are new bands meeting old code, and clearing them is a refactor of the source tree rather than a step in the phase that drew them. |
