...
| # | Test | Verifies | Purpose |
| - | ---- | -------- | ------- |
| 1 | <a id="a_file_the_mapping_describes_is_covered"></a>`a_file_the_mapping_describes_is_covered` | `LLR-DWL-05` | A file the coverage set names is covered and one it does not name is not — the test that governs every other query in the module. |
| 2 | <a id="an_empty_set_covers_nothing"></a>`an_empty_set_covers_nothing` | `LLR-DWL-04` | An image carrying no debug information at all covers no file, so nothing is pruned and the run reports what it reported before this mechanism existed. Null arguments answer the same way rather than faulting. |
| 3 | <a id="a_line_the_mapping_names_was_compiled"></a>`a_line_the_mapping_names_was_compiled` | `LLR-DWL-03` | Membership at the first line, the last, and one in the middle; a gap between named lines, a line before the first and one past the last all answer not compiled. The binary search is what a heavily repeated line programme is collapsed to, so its boundaries are worth pinning. |
| 4 | <a id="an_uncovered_file_answers_not_compiled_for_every_line"></a>`an_uncovered_file_answers_not_compiled_for_every_line` | `LLR-DWL-05` | **The failure the two-part contract exists to prevent.** A caller asking only this question would find every line of an uncovered file uncompiled and delete the file. The answer is deliberately the unsafe one; the coverage call is what makes asking it safe, and it has to be asked first. |
| 5 | <a id="several_files_are_each_looked_up_on_their_own_lines"></a>`several_files_are_each_looked_up_on_their_own_lines` | `LLR-DWL-03` | One file's lines are not another's, over a set sorted by path — which is what the binary search over the file table requires. |
| 6 | <a id="a_covered_file_with_no_lines_compiled_nothing"></a>`a_covered_file_with_no_lines_compiled_nothing` | `LLR-DWL-05` | Covered and empty is a different claim from uncovered, and both are representable: the mapping described this file and recorded no instruction for any line of it. |
...
| 47 | <a id="an_image_path_is_recorded_unvalidated"></a>`an_image_path_is_recorded_unvalidated` | `LLR-CLI-22` | The path reaches the options as given and nothing is opened. Whether the file is an image elc can read belongs to the module that owns the failure; answering it in the parser would put a second copy of that knowledge there. |
| 48 | <a id="no_image_is_the_default"></a>`no_image_is_the_default` | `LLR-CLI-22` | With no `--elf` the image path is null, which is how adding the option changes no existing result. |
| 49 | <a id="an_empty_image_path_is_a_usage_error"></a>`an_empty_image_path_is_a_usage_error` | `LLR-CLI-22` | An empty argument names no file, so there is nothing for the reader of the image to diagnose and the parser rejects it itself. |
| 50 | <a id="an_image_with_regeneration_is_a_usage_error"></a>`an_image_with_regeneration_is_a_usage_error` | `LLR-CLI-23` | The filter is applied when a file is measured, so a saved record already describes one filtered run and cannot be re-cut against another image. Rejected rather than silently ignored. |
| 51 | <a id="the_usage_summary_documents_the_image_option"></a>`the_usage_summary_documents_the_image_option` | `LLR-USG-07` | The summary names the option and states that no image means no filtering. The documentation test checks the manual and man page against this text, so this is where the claim itself is made. |
| 52 | <a id="an_output_extension_selects_the_format"></a>`an_output_extension_selects_the_format` | `LLR-CLI-26` | Each recognised extension resolves to its own format with no format option given, driven as a table so a format added to the mapping without a case here is visible. |
| 53 | <a id="an_unrecognised_output_extension_is_a_usage_error"></a>`an_unrecognised_output_extension_is_a_usage_error` | `LLR-CLI-26` | `-o report.json` is rejected rather than defaulted to the table, which would write a file whose name promises JSON and whose contents are not. |
| 54 | <a id="an_output_path_with_no_extension_is_a_usage_error"></a>`an_output_path_with_no_extension_is_a_usage_error` | `LLR-CLI-26` | The four spellings of "no extension": a bare name, a dotfile, a trailing dot, and a name whose only dot is in a directory component. Each names no format and is rejected. |
| 55 | <a id="a_format_option_contradicting_the_filename_is_a_usage_error"></a>`a_format_option_contradicting_the_filename_is_a_usage_error` | `LLR-CLI-27` | Driven in both argument orders, because the comparison happens after the option loop rather than as the options arrive; an implementation comparing them in place would pass one order and fail the other. |
| 56 | <a id="a_format_option_agreeing_with_the_filename_is_accepted"></a>`a_format_option_agreeing_with_the_filename_is_accepted` | `LLR-CLI-27` | Nothing is ambiguous about saying a thing twice, so the agreeing pair parses and yields the format both named. |
| 57 | <a id="the_format_defaults_to_the_table"></a>`the_format_defaults_to_the_table` | `LLR-CLI-02`, `LLR-CLI-27` | The default has to survive a resolution step that now runs whether or not anything selected a format, so it is asserted directly rather than left implied by the tests that do select one. |
| 58 | <a id="standard_output_takes_its_format_from_the_option_alone"></a>`standard_output_takes_its_format_from_the_option_alone` | `LLR-CLI-27`, `LLR-CLI-02` | With no output path there is no extension to resolve, so the option decides and the destination stays standard output. |
| 59 | <a id="regenerating_into_a_filename_naming_a_table_is_a_usage_error"></a>`regenerating_into_a_filename_naming_a_table_is_a_usage_error` | `LLR-CLI-28`, `LLR-CLI-10` | An output extension is an explicit format selection, so regeneration rejects one naming anything but Markdown exactly as it rejects `-f table`. |
| 60 | <a id="regenerating_into_a_filename_naming_markdown_is_accepted"></a>`regenerating_into_a_filename_naming_markdown_is_accepted` | `LLR-CLI-28`, `LLR-CLI-10` | The complement, and what stops the rule above from rejecting every `--output` in regeneration mode. |
| 61 | <a id="the_summary_is_the_default_verbosity"></a>`the_summary_is_the_default_verbosity` | `LLR-CLI-29` | A parsed command line with no verbosity option leaves the flag clear, so a zeroed options structure means the composition a run presents when nothing was asked for. |
| 62 | <a id="verbose_is_recorded_in_either_spelling"></a>`verbose_is_recorded_in_either_spelling` | `LLR-CLI-29` | Both the short and the long spelling reach the options structure. |
| 63 | <a id="verbose_with_a_complete_record_format_is_accepted"></a>`verbose_with_a_complete_record_format_is_accepted` | `LLR-CLI-30` | The pairing that is not a usage error. Both the request and the format survive the parse for XML and for CSV alike, so the acceptance is a property of the rule rather than of one format. |
| 64 | <a id="the_matrix_companion_is_off_unless_requested"></a>`the_matrix_companion_is_off_unless_requested` | `LLR-CLI-31` | A zeroed configuration means no companion, as it does for the GraphML export. |
| 65 | <a id="the_matrix_companion_is_requested_without_an_output_path"></a>`the_matrix_companion_is_requested_without_an_output_path` | `LLR-CLI-31` | Recorded, not validated against `--output`. Asking for the matrix with the report on standard output writes no file and is not a usage error; rejecting it would make the invocation fail where the requirement says it should quietly produce no companion. |
| 66 | <a id="the_matrix_companion_is_accepted_with_regeneration"></a>`the_matrix_companion_is_accepted_with_regeneration` | `LLR-CLI-31` | Where `--graphml` is rejected with `--from-xml`, this one is accepted: a saved record carries the matrix where it carries no graph, so there is something to write from. |
| 67 | <a id="the_usage_summary_documents_verbosity_and_the_extension_rule"></a>`the_usage_summary_documents_verbosity_and_the_extension_rule` | `LLR-USG-09`, `LLR-USG-02` | The summary names the verbosity option and states that an output filename's extension selects the format. The documentation test checks both delivered documents against this text, so this is where the claim itself is made. Reading the text back out of a caller-supplied stream is also what establishes that the summary is written to the stream it was handed rather than to a fixed one. |
| 68 | <a id="the_purification_thresholds_default_to_elcs_own"></a>`the_purification_thresholds_default_to_elcs_own` | `LLR-CLI-32` | With none supplied the five purification thresholds take the defaults that make purification mean anything at all — a zeroed set would classify every function as everything. |
| 69 | <a id="every_purification_threshold_is_configurable"></a>`every_purification_threshold_is_configurable` | `LLR-CLI-32` | Each of the five is settable, because a heuristic that cannot be adjusted is one whose disagreements have nowhere to go. |
...
| 8 | <a id="a_components_directory_is_the_path_above_it"></a>`a_components_directory_is_the_path_above_it` | `LLR-DIR-01` | The directory is the path preceding the last separator, carrying no trailing one, so two files in one directory compare equal. |
| 9 | <a id="a_file_at_the_root_belongs_to_the_root"></a>`a_file_at_the_root_belongs_to_the_root` | `LLR-DIR-01` | The first of the two edge cases a split on the last separator gets wrong: the empty prefix is the root, not nothing. |
| 10 | <a id="a_path_with_no_separator_belongs_to_the_working_directory"></a>`a_path_with_no_separator_belongs_to_the_working_directory` | `LLR-DIR-01` | The second: a path naming no directory belongs to the one it is relative to. |
...
| 5 | <a id="the_summary_omits_the_per_function_tier"></a>`the_summary_omits_the_per_function_tier` | `LLR-SUM-09` | The tier boundary at its sharpest: a file's own totals stay in the summary and the per-function table goes, and the verbose render of the same model has it back. |
| 6 | <a id="the_summary_keeps_the_findings"></a>`the_summary_keeps_the_findings` | `LLR-SUM-09` | The findings tier is emitted at the summary verbosity whether or not it has rows. A summary that dropped the one section a reader acts on would be shorter and useless. |
| 7 | <a id="the_verbose_report_is_a_superset_of_the_summary"></a>`the_verbose_report_is_a_superset_of_the_summary` | `LLR-SUM-09` | The verbose render of one model is strictly longer than its summary, so the verbosity selects something rather than being accepted and ignored. |
...
| 16 | <a id="HLR-006: the report has the same sections whatever the target type"></a>`HLR-006: the report has the same sections whatever the target type` | — | A file target and a directory target produce the same sections in the same order. |
| 17 | <a id="HLR-015: each function reports its own ELOC"></a>`HLR-015: each function reports its own ELOC` | — | A function's own ELOC appears beside its line range. |
| 18 | <a id="HLR-024: the project summary carries a combined ELOC total"></a>`HLR-024: the project summary carries a combined ELOC total` | — | The summary reports the combined ELOC across every analysed file. |
| 19 | <a id="HLR-025: the totals are broken down by language"></a>`HLR-025: the totals are broken down by language` | — | The report carries a per-language section, so each language's contribution is separately visible. |
| 20 | <a id="HLR-025: the per-language totals sum to the project totals"></a>`HLR-025: the per-language totals sum to the project totals` | — | With one language present, its row equals the summary exactly — the breakdown is a partition of the totals, not a second count of them. |
| 21 | <a id="HLR-019: a header of declarations only reports zero ELOC"></a>`HLR-019: a header of declarations only reports zero ELOC` | — | A file's own line and ELOC counts are reported per file, and a header whose only statement is one return reports one. |
| 22 | <a id="HLR-032: two runs over a parsed tree are byte-identical"></a>`HLR-032: two runs over a parsed tree are byte-identical` | — | Repeating a run that parses produces identical bytes, so the new sections are as deterministic as the old ones. |
| 23 | <a id="HLR-011: every delivered language is detected from its extension"></a>`HLR-011: every delivered language is detected from its extension` | — | One target holding a file of each delivered language yields all five in the per-language breakdown. |
| 24 | <a id="HLR-008: a mixed-language target is analysed in one invocation"></a>`HLR-008: a mixed-language target is analysed in one invocation` | — | Four languages are analysed in a single pass, with every function found — no invocation per language. |
| 25 | <a id="HLR-025: each language's contribution is separately visible"></a>`HLR-025: each language's contribution is separately visible` | — | A two-language target reports both languages, in name order. |
| 26 | <a id="HLR-011: elc requires no particular language to be present"></a>`HLR-011: elc requires no particular language to be present` | — | A target of one language runs exactly as a mixed one does; nothing verifies that the other four are installed. |
| 27 | <a id="HLR-035: a file with a syntax error is measured around it"></a>`HLR-035: a file with a syntax error is measured around it` | `LLR-ANL-48` | The report covers the functions the grammar could follow in a damaged file. This asserted the opposite until a real project showed that the discard-everything rule cost half its metrics. |
| 28 | <a id="HLR-035: the damage is reported beside the figures it qualifies"></a>`HLR-035: the damage is reported beside the figures it qualifies` | `LLR-RPT-30` | The safety property that makes measuring a damaged file acceptable: the file is named in a section of its own and the unparsed total appears in the summary, so a partial measurement never reads as a complete one. |
...
| 14 | <a id="HLR-030: every format honours --output"></a>`HLR-030: every format honours --output` | `LLR-MAIN-12` | Redirection applies to every format, not only the default one, and the results reach the named file with the skipped-file notice on standard error and standard output left empty. |
...
| 17 | <a id="HLR-148: each recognised extension selects its format with no option"></a>`HLR-148: each recognised extension selects its format with no option` | — | Each of `.txt`, `.md`, `.csv`, and `.xml` produces the format it names with no format option given, identified by a marker only that format emits. The filename has already said what the format is, and nothing should have to say it twice. |
| 18 | <a id="HLR-148: an unrecognised extension is a usage error naming both"></a>`HLR-148: an unrecognised extension is a usage error naming both` | — | An output named `report.json` is rejected with exit status 2, the diagnostic names the extension found and every extension that would have worked, and no file is written. Guessing would produce a `report.json` holding no JSON. |
| 19 | <a id="HLR-148: a filename with no extension is a usage error too"></a>`HLR-148: a filename with no extension is a usage error too` | — | A path carrying no extension names no format, and is rejected rather than defaulted to the table. |
| 20 | <a id="HLR-148: a dotfile has no extension, and a trailing dot names nothing"></a>`HLR-148: a dotfile has no extension, and a trailing dot names nothing` | — | The two boundary spellings. A leading dot names a hidden file rather than an extension, and a trailing dot names nothing at all; both are rejected as having none. |
| 21 | <a id="HLR-148: a directory carrying a dot does not lend its extension"></a>`HLR-148: a directory carrying a dot does not lend its extension` | — | The extension is the last dot of the *basename*. A `build.d/report` has none of its own, and reading one out of the directory component would pick up `.d`. |
| 22 | <a id="HLR-149: a format option contradicting the filename is a usage error"></a>`HLR-149: a format option contradicting the filename is a usage error` | — | `-f csv -o report.md` is rejected with both named in the diagnostic and nothing written. Honouring either would leave the user's own command line disagreeing with the file it produced. |
| 23 | <a id="HLR-149: a format option agreeing with the filename is accepted"></a>`HLR-149: a format option agreeing with the filename is accepted` | — | Nothing is ambiguous about saying a thing twice, so the agreeing pair is accepted and produces the format both named. This is what stops the rejection above from being a rejection of `-f` with `-o` altogether. |
| 24 | <a id="HLR-149: with no output file the option still selects the format"></a>`HLR-149: with no output file the option still selects the format` | — | Standard output has no filename and so no extension, which is what keeps a machine-readable format available to a caller that pipes rather than redirects. |
| 25 | <a id="HLR-148: the companion artefacts keep their own extensions"></a>`HLR-148: the companion artefacts keep their own extensions` | — | The extension governs the format alone: an `--output` of `report.md` still yields `report.dot` and `report.graphml`, because the companion naming substitutes its own extension on the same path. |
...
| 4 | <a id="HLR-115: an omitted analysis states its reason in the summary too"></a>`HLR-115: an omitted analysis states its reason in the summary too` | — | The omission notices are summary tiers even though the sections carrying them are detail tiers. An analysis nobody declared for is something the reader must be told at either verbosity, rather than an absence they might read as a clean bill of health. |
| 5 | <a id="HLR-150: an analysis that was measured is not in the summary"></a>`HLR-150: an analysis that was measured is not in the summary` | — | The converse of the notice above, and what stops that test passing against a renderer that simply always emits the section: with entry points declared, the reachability sections are detail tiers again and are omitted. |
| 6 | <a id="HLR-151: --verbose presents the detail tiers as well"></a>`HLR-151: --verbose presents the detail tiers as well` | — | Every tier the default omits is present with the option given. |
| 7 | <a id="HLR-151: the verbose report is the summary plus the detail tiers"></a>`HLR-151: the verbose report is the summary plus the detail tiers` | — | Stated as a superset over the headings actually emitted rather than as a list, so that a tier added to the traversal in a later phase cannot satisfy one composition and be forgotten in the other. The strict-inequality half keeps it from passing where the option selects nothing. |
| 8 | <a id="HLR-151: -v is the short form of --verbose"></a>`HLR-151: -v is the short form of --verbose` | — | Both spellings produce identical bytes. |
| 9 | <a id="HLR-151: verbosity changes no measurement and no exit status"></a>`HLR-151: verbosity changes no measurement and no exit status` | `LLR-MAIN-21` | A value absent from a summary is absent because it was not printed, never because it was not computed — so the record, which carries every measurement either composition can present, is byte-identical, and so is the exit status. |
| 10 | <a id="HLR-031: both human formats present the same tiers at each verbosity"></a>`HLR-031: both human formats present the same tiers at each verbosity` | — | Uniformity across formats holds at a fixed verbosity and is asserted at both, which is what the amended HLR-031 requires. The two are compared heading by heading with Markdown's decoration stripped. |
| 11 | <a id="HLR-152: --verbose with an xml output is accepted, not rejected"></a>`HLR-152: --verbose with an xml output is accepted, not rejected` | — | The one option pairing this project defines that is not a usage error. There is nothing contradictory about asking a complete format for detail, so the invocation succeeds. |
| 12 | <a id="HLR-152: --verbose with a csv output is accepted, not rejected"></a>`HLR-152: --verbose with a csv output is accepted, not rejected` | — | The same rule for the other complete format, stated separately because HLR-152 reaches it for the same reason rather than a different one. |
| 13 | <a id="HLR-152: csv is byte-identical at either verbosity"></a>`HLR-152: csv is byte-identical at either verbosity` | `LLR-MAIN-21` | Acceptance is not enough: the file produced must be the same one. A summarised CSV would be a record set with no rows in it. This is also where the dispatch is checked: the verbosity reaches the two human-facing renderers and neither complete-record writer, so a CSV that changed with it would mean the parameter had been threaded somewhere it does not belong. |
| 14 | <a id="HLR-056: a record regenerated summarily matches a direct summary run"></a>`HLR-056: a record regenerated summarily matches a direct summary run` | — | Verbosity joins the threshold as a property of the rendering: one record answers a summary question identically to a direct run. |
...
| 17 | <a id="HLR-048: C++ counts exception handling toward ELOC"></a>`HLR-048: C++ counts exception handling toward ELOC` | — | The one ELOC category C cannot express. `try`, `catch`, and `throw` each contribute a line, which no earlier phase could verify in any shipped language. |
...
| 16 | <a id="HLR-139: a language with no dead-code query is reported not analysed"></a>`HLR-139: a language with no dead-code query is reported not analysed` | `LLR-DED-05` | Every shipped module supplies one, so the case is built: a runtime is copied and the file removed, and the heading names the language rather than leaving a clean-looking table. |
...
| 7 | <a id="HLR-155: both counts are reported"></a>`HLR-155: both counts are reported` | `LLR-SUM-06` | Three statements and the two directives around them pruned; one file whose coverage could not be established. The pair is read as the unresolved-call count and the undecided-region count are — the first states what the filter removed, the second where it could not look. |
| 8 | <a id="HLR-133: the region is undecided, so only the image ruled it out"></a>`HLR-133: the region is undecided, so only the image ruled it out` | — | The fixture's whole assertion rests on this. Were the condition decidable from the source, Phase 15 would have removed the region before the image was consulted and every pruning test would pass against an implementation that read no debug information at all. |
| 9 | <a id="HLR-153: an image without debug information prunes nothing"></a>`HLR-153: an image without debug information prunes nothing` | `LLR-DWL-04` | HLR-141 forbids *requiring* debug information, so its absence costs the finer granularity and nothing else: every ELOC figure is the unfiltered one, nothing is pruned, and both files are counted uncovered rather than silently emptied. |
| 10 | <a id="HLR-153: the metrics without debug information are the unfiltered ones"></a>`HLR-153: the metrics without debug information are the unfiltered ones` | `LLR-DWL-04` | Stated as an equality between two runs rather than left to two hand-counted tables: the figures reported for an image carrying no line information are the figures reported before this phase existed. |
...
| 13 | <a id="LLR-DWL-02: a path with .. components still resolves to the file"></a>`LLR-DWL-02: a path with .. components still resolves to the file` | `LLR-DWL-02` | The compiler records the file name as the command line spelled it, joined to the unit's compilation directory, so an ordinary build records a name carrying `.` and `..`. The image's spelling and elc's canonical path meet only if the join is normalised — lexically, never through `realpath(3)`, which would stat every path the image happens to name. Coverage is established despite the components, and the guarded region is pruned, which it could not be from an uncovered file. |
| 14 | <a id="HLR-155: the counts survive a record round trip"></a>`HLR-155: the counts survive a record round trip` | `LLR-XWR-14` | Neither count can be recomputed from a record: regeneration has no image, and no debug information to read from one. |
| 15 | <a id="HLR-140: with no image nothing is filtered and no section is added"></a>`HLR-140: with no image nothing is filtered and no section is added` | `LLR-ANL-54`, `LLR-SUM-06` | The opt-in promise, and the cheapest regression test available. An empty section is not nothing, so the uniform-composition rule gives way here and the filter sections are absent entirely. |
| 16 | <a id="HLR-140: an unfiltered run is byte-identical across formats and runs"></a>`HLR-140: an unfiltered run is byte-identical across formats and runs` | — | Two unfiltered runs over one tree produce the same bytes, so nothing the option added reaches a run that did not ask for it. |
| 17 | <a id="HLR-143: the source functions the image does not define are listed"></a>`HLR-143: the source functions the image does not define are listed` | `LLR-RPT-31` | The finding the option exists to produce: both functions of the translation unit the link never included, named. |
| 18 | <a id="HLR-143: an absent function is located, not merely named"></a>`HLR-143: an absent function is located, not merely named` | `LLR-RPT-31` | The reader's next action is to open the file, so a name with no line is a finding they cannot act on. |
| 19 | <a id="HLR-143: the report is not a dead-code claim about the call graph"></a>`HLR-143: the report is not a dead-code claim about the call graph` | — | Two lists, two claims. A function the linker discarded is observed; an unreachable function is inferred from a traversal. The reachability section is untouched by the filter and still states its own omission. |
| 20 | <a id="LLR-ELF-02: a function the image only imports is not one it defines"></a>`LLR-ELF-02: a function the image only imports is not one it defines` | `LLR-ELF-02` | The image carries a printf of function type whose section index is `SHN_UNDEF`. Without the definedness test a source definition of printf would be retained, and the filter would keep code the build never compiled. |
| 21 | <a id="HLR-144: a call whose target was filtered out is unresolved"></a>`HLR-144: a call whose target was filtered out is unresolved` | — | The callee is in the image's import list and not its definitions, so the call resolves to nothing. Inventing the edge would make the reachability claim unsound in the one direction it is not already known to err. |
| 22 | <a id="LLR-ANL-52: a filtered function contributes no fact of any kind"></a>`LLR-ANL-52: a filtered function contributes no fact of any kind` | `LLR-ANL-52` | An omitted function is in no graph-derived table either: not a node, not a fan-out row, not a decision point on anything. |
| 23 | <a id="HLR-145: file-scope ELOC is retained and reported separately"></a>`HLR-145: file-scope ELOC is retained and reported separately` | `LLR-ANL-53` | Two lines of file-scope data across the two files, reported as a figure of their own — the part of the total the filter did not narrow. |
| 24 | <a id="HLR-145: a file the image kept nothing from still reports its data"></a>`HLR-145: a file the image kept nothing from still reports its data` | `LLR-ANL-53` | A file with no function left reports its one line of file-scope ELOC, so a reader can tell a file of retained functions from a file of retained data. |
| 25 | <a id="HLR-142: a C++ image matches through Itanium demangling"></a>`HLR-142: a C++ image matches through Itanium demangling` | `LLR-SYM-02`, `LLR-SYM-03` | Every name but main reaches the image encoded and qualified. Matching raw linkage names would retain nothing at all, and matching a demangled one without reducing it would match nothing either. |
| 26 | <a id="HLR-143: a name in a scheme this build cannot decode is counted"></a>`HLR-143: a name in a scheme this build cannot decode is counted` | `LLR-SYM-04`, `LLR-ELF-08` | A v0-mangled symbol linked into an otherwise ordinary image raises the unresolved count by one and leaves the rest of the filter working: one symbol resolved to nothing, not the image. |
| 27 | <a id="HLR-146: a stripped image is fatal and diagnosed as its own case"></a>`HLR-146: a stripped image is fatal and diagnosed as its own case` | `LLR-ELF-07` | The case that most needs to be fatal. An empty function set would filter every function away and report a project containing none — confidently wrong, and indistinguishable from a correct result. |
| 28 | <a id="HLR-146: a file that is not an object file is fatal"></a>`HLR-146: a file that is not an object file is fatal` | `LLR-ELF-06` | A source file named where an image was meant ends the run with a diagnostic naming it and no report. |
| 29 | <a id="HLR-146: an absent image is fatal and names the path"></a>`HLR-146: an absent image is fatal and names the path` | `LLR-ELF-06` | The user named the file, so the failure is theirs to correct and the diagnostic says which file. |
| 30 | <a id="HLR-146: an unusable image ends the run before anything is measured"></a>`HLR-146: an unusable image ends the run before anything is measured` | `LLR-MAIN-20` | Fatal before discovery, not after a walk whose results are then thrown away: nothing about the source tree reaches stderr either. |
| 31 | <a id="HLR-147: the report names the image it was filtered by"></a>`HLR-147: the report names the image it was filtered by` | `LLR-RPT-34`, `LLR-SUM-06` | A report that filtered and did not say which image it filtered by cannot be checked against the build it claims to describe. |
| 32 | <a id="HLR-147: --elf with --from-xml is a usage error"></a>`HLR-147: --elf with --from-xml is a usage error` | `LLR-CLI-23` | The filter is applied when a file is measured, so a record already describes one filtered run. Rejected rather than silently ignored, the same rule a definition gets. |
| 33 | <a id="HLR-147: a report regenerated from the record is byte-identical"></a>`HLR-147: a report regenerated from the record is byte-identical` | `LLR-XRD-14` | A record written from a filtered run regenerates into the same Markdown the direct run produced, filter sections included. |
| 34 | <a id="HLR-147: the record carries the image and both mismatch counts"></a>`HLR-147: the record carries the image and both mismatch counts` | `LLR-XWR-14` | The image element and one absent element per function the image lacks, in a well-formed document. Without them a regenerated report would describe a filtered run while naming no filter. |
| 35 | <a id="HLR-140: an unfiltered record carries no image element"></a>`HLR-140: an unfiltered record carries no image element` | `LLR-XWR-14` | An unfiltered run writes exactly the record it wrote before the option existed. |
...
| `LLR-ANL-58` | `analyze_file` | `HLR-144`, `HLR-143` | `LLR-ANL-58: a function no configuration builds is not one the image lacks` |
...
| `LLR-SUM-02` | `render_summary` | `HLR-031` | `both_styles_reach_the_same_tiers_at_each_verbosity` |
