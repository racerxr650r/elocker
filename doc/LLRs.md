...
*   <a id="LLR-HTM-04"></a>**LLR-HTM-04** — `format_html` shall emit an initialisation script that constructs the viewer over the embedded payload with no layout of its own, initialises the expand-collapse extension with its fisheye and animation behaviours enabled, collapses every container, and only then runs a force-directed layout — so the one layout the page opens with is computed over the collapsed view it opens showing.
    ...
    **It shall be handed to the expand-collapse extension rather than invoked around it.** An expansion that animates is asynchronous, so a layout called on the line after it runs while the children are still arriving and positions nothing: every function of an opened file is left stacked at a single point, which is what the page did when this script ran its own layout. The extension's layout hook runs after an expansion and after a collapse — the moment this script cannot name for itself — and the initial collapse then carries the drawing's first layout with it, so nothing lays the graph out twice. This is the same class of mistake as fitting the view between two asynchronous layouts, made a second time at a different seam, which is why the requirement now states the rule rather than the instance. A call graph is read as a hierarchy — who calls whom, in which direction — which is how the `.dot` companion is drawn and why it reads as an architecture rather than a mesh; a force-directed arrangement of the same graph is a hairball however the edges run, and was the first thing a reader noticed between the two artefacts.
