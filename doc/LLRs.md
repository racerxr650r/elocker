...
*   <a id="LLR-HTM-06"></a>**LLR-HTM-06** — `format_html` shall emit a stylesheet giving each annotation of LLR-CYT-05 a distinct visual attribute — fill for severity, shape for the role a function takes part in, border for whether it is reached and for the deepest chain — and a legend naming every one of them.
    ...
    **An edge shall pass behind a box rather than across its face.** A node that becomes a container is drawn at a lower compound depth than an edge between two nodes outside it, so an opened file otherwise has the rest of the drawing's edges laid over it — the one place in the drawing where a reader most needs to see what is inside. Placing every edge at the bottom of the compound order restores the behaviour the drawing has everywhere else, where an edge that crosses a box passes behind it.
