...
# **Every case here runs `--no-expand`**, and that is the point rather than a
# convenience. Repair is the path a file takes when the preprocessor could not
# be used — no toolchain, a header the host cannot find, a condition elc could
# not decide — and a test that let expansion run would be testing expansion.
# `--no-expand` is the one switch that puts every file on that path
# deliberately (HLR-196).
...
@test "LLR-RPR-01: a file with nothing to repair is untouched" {
	elc --no-expand --dbg -o "$OUT" "$TREE/sound.c"
	assert_success
	run cat "$DBG"
...
@test "LLR-RPR-05: two runs over one target repair identically" {
	...
	elc --no-expand --dbg -o "$BATS_TEST_TMPDIR/a.md" "$TREE"
	local first
	first="$(grep -c 'repair:' "$BATS_TEST_TMPDIR/a.dbg")"
	elc --no-expand --dbg -o "$BATS_TEST_TMPDIR/b.md" "$TREE"
...
@test "LLR-RPR-06, HLR-199: each repair is recorded with its rule" {
	elc --no-expand --dbg -o "$OUT" "$TREE/shapes.c"
	assert_success
	run cat "$DBG"
...
@test "HLR-196: an expanded file is not repaired, an unexpanded one is" {
	# The two paths are exclusive, and the report says which each file
	# took. Expansion is exact and needs a toolchain; repair needs nothing
	# and is a guess. Where the first applies the second is unnecessary;
	# where it does not, the second is all there is.
...
@test "HLR-196: both paths measure the same functions" {
	# The claim that makes the fallback worth having rather than merely
	# better than nothing: a file the preprocessor expanded and the same
	# file repaired report the same functions, with the same figures.
	require_tool gcc "HLR-196 repair as the fallback"
	gcc -E -C "$TREE/shapes.c" >/dev/null 2>&1 || skip "gcc cannot preprocess here"

	elc --verbose "$TREE/shapes.c"
	assert_success
	local expanded
	expanded="$(figures branchy) $(figures report)"
...
@test "LLR-RPR-04: a pass that does not help is withdrawn, not left half-done" {
	...
	elc --no-expand --verbose "$BATS_TEST_TMPDIR/bad.c"
	local damaged
	damaged="$(figures g)"

	elc --no-expand --verbose "$BATS_TEST_TMPDIR/good.c"
	assert_success
	assert_equal "$damaged" "$(figures g)"

	# And no repair was kept, because none of them helped.
	elc --no-expand --dbg -o "$BATS_TEST_TMPDIR/bad.md" "$BATS_TEST_TMPDIR/bad.c"
