...
/* --------------------------------------------------- the testing burden --
 *
 * The page is given the *decision*, not the bounds. Emitting the index and
 * letting the stylesheet compare it against 20 and 45 would put the
 * thresholds in a second place, in a script, where nothing checks them
 * against the catalogue that decided the text report's finding — and the two
 * would disagree the first time a bound moved (LLR-CYT-06).
 */
                       ...
                       double tbi)
{
	...
	cr_assert_fail("no function named %s", name);
}
...
Test(report_html, a_function_node_carries_mock_burden_tbi_and_status)
{
	...
	scene_build(&s, PATHS, FUNCTIONS, 3, NULL, NULL, 0);
	set_burden(&s.report, "app_fn", 0.85, 21.5);
	page    = page_of(&s.report, &s.graph, &opts);
	payload = payload_of(page);
...
/* Verifies LLR-CYT-06: the string is the band the catalogue would decide, on
 * both sides of both bounds — so a page cannot colour a node differently from
 * the way the report describes it. */
Test(report_html, the_status_string_agrees_with_the_band_the_catalogue_decided)
{
	static const struct {
		...
		{  0.0, "healthy"  },
		{ 19.9, "healthy"  },   /* under the warning bound        */
		{ 20.0, "warning"  },   /* exactly on it, and inclusive   */
		{ 44.9, "warning"  },   /* under the critical bound       */
		{ 45.0, "critical" },   /* exactly on it, and inclusive   */
	};
